...
$j(document).ready(function () {
        ...
        var misc_titles = "Social Activity*High-Tech Purchase*Painkillers*Birthday Gift*Pair of Shoes*Barber Fees*Streaming Service*National Park*New Book*Yard Sale*Clothing Shop*Concert Tickets".split("*");
        var misc_texts = "Your friends are going to watch the latest action flick at the IMAX theater in town, then going out for drinks after. Do you join them?*A tech product you've had your eyes on for a long time is sold at a 50% discount this week. Do you buy it?*You wake up with a bad tooth ache, and you are out of painkillers at home. Do you go to the pharmacy to buy painkillers?*Your best friend is turning 30 this week, you have have the perfect gift idea for him. Do you buy it?*You stumble upon a very nice pair of leather shoes, which would be perfect for the fall and winter. Do you buy them?*It has been a while since you last had a haircut, and a nice barbershop has opened near your house. Do you try it?*A video streaming website is offering a one-time deal: -50% on its yearly subscription. Do you accept the deal?*You are considering going on a hike to a National Park this week-end, but you need to pay for the entrance and camping fees. Do you go?*Your favorite author just released a new book. Do you buy a copy of it?*Your neighbors are doing a yard sale, and you spot a nice wooden desk at a very attractive price. Do you buy it?*You have not purchased new clothes in a while, and you just happen to have a date this week. Would you use the occasion to buy a nice shirt and a pair of pants?*A musician that you like a lot is touring in town next month. Do you buy tickets to the show?".split("*");
        var misc_prices = "35*120*20*40*100*30*60*45*15*60*100*30".split("*");
        var misc_declined = "You will not join your friends*You will not buy the tech product*You will not buy painkillers*You will not buy the birthday present*You will not buy the shoes*You will not get a haircut*You will not subscribe to the streaming service*You will not go to the National Park*You will not buy the book*You will not buy the wooden desk*You will not buy new clothes*You will not attend the concert".split("*");
        ...
        $j("#tabs").tabs();
        $j(".txt-week").text(week + 1);

        function initRecurringPage() {
            var fundpastbalanceprompt = '';
            var fundpastbalancebutton = '';
            var salarypastbalanceprompt = '';
            var salarypastbalancebutton = '';
            ...
            if (fundpastbalance > 0) {
                fundpastbalanceprompt = 'You finished the week with $<span id="txt-fundpastbalance">400</span> left on ' +
                    'your <span class="txt-fundtype">food</span> card.';
                fundpastbalancebutton = 'Add the <span class="txt-fundtype">food</span> card balance'
            } else {
                fundpastbalanceprompt = 'You have depleted your <span class="txt-fundtype">food</span> card. Nothing ' +
                    'to add to your balance.';
                fundpastbalancebutton = 'Understood'
            }

            if (salarypastbalance > 0) {
                salarypastbalanceprompt = 'You finished the week with $<span id="txt-salarypastbalance">400</span> left ' +
                    'on your bank account.';
                salarypastbalancebutton = 'Add the bank account balance'
            } else {
                salarypastbalanceprompt = 'You finished the week with $0 left on your bank account. Nothing to add to ' +
                    'your balance.';
                salarypastbalancebutton = 'Understood'
            }

            $j('#fundpastbalanceprompt').html(fundpastbalanceprompt);
            $j('#btn-cashfundpastbalance').html(fundpastbalancebutton);
            $j('#salarypastbalanceprompt').html(salarypastbalanceprompt);
            $j('#btn-cashsalarypastbalance').html(salarypastbalancebutton);

            $j('#txt-fundpastbalance').text(fundpastbalance);
            $j('#txt-salarypastbalance').text(salarypastbalance);
            $j('#txt-salary').text(salary);
            $j('#txt-rent').text(rent);

            $j("#panel-salarypastbalance").find('.panel-body').slideUp();
            $j("#panel-salary").find('.panel-body').slideUp();
            $j("#panel-rent").find('.panel-body').slideUp();


            $j("#btn-cashfundpastbalance").button().click(function () {
                $j("#btn-cashfundpastbalance").button("option", "disabled", true);
                addMoneyTo("Fund", fundpastbalance);
                addRow("Income", fundtypecaps + " Card Balance", 0, fundpastbalance);
                refreshTotal();
                $j("#panel-fundpastbalance").find('.panel-body').slideUp();
                $j("#panel-salarypastbalance").find('.panel-body').slideDown();
            });


            $j("#btn-cashsalarypastbalance").button().click(function () {
                $j("#btn-cashsalarypastbalance").button("option", "disabled", true);
                addMoneyTo("Card", salarypastbalance);
                addRow("Income", "Past Account Balance", salarypastbalance, 0);
                refreshTotal();
                $j("#panel-salarypastbalance").find('.panel-body').slideUp();
